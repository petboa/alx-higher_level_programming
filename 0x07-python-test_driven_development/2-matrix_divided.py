...
def matrix_divided(matrix_list, divider):
    ...
    Raises:
        TypeError: if matrix_list is not a
        list of lists or does not contain integers/floats.
    ...
    if not isinstance(matrix_list, list) or len(matrix_list) == 0 or not 
    all(isinstance(i, list) and all(isinstance(j, (int, float)) for j in i) for i in matrix_list):
